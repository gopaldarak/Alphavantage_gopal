...
public abstract class TechnicalIndicatorParser<Data> extends JsonParser<Data> {
  ...
  protected LocalDateTime resolveDate(String key) {
    switch (interval) {
      ...
      case WEEKLY:
      case DAILY:
        return LocalDate.parse(key.substring(0, 10), SIMPLE_DATE_FORMAT).atStartOfDay();
      default:
        return LocalDateTime.parse(key.substring(0, 10), DATE_WITH_SIMPLE_TIME_FORMAT);
    }
  }

  @Override
  protected Data resolve(JsonObject rootObject) {
	  if (rootObject.get(getIndicatorKey()) == null){
		  System.out.println("Root obj is null");
	  }
