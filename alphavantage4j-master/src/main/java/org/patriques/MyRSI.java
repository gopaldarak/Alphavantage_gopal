...
public class MyRSI {
	...
	private static AlphaVantageConnector apiConnector = new AlphaVantageConnector(apiKey,
			timeout);
					...
					int size = priceList.size() > rsiList.size() ? rsiList
						...
						for (int i = 0; i < size; i++) {
							...
							rsiPriceList.add(new RSIPrice(myStock, price
									.getDateTime(), rsi.getValue(),
									stochDataSlow.getSlowK(),
									stochDataSlow.getSlowD(),
									price.getClose()));
						}
...
//Todo-get oldest data for +/- 5 days to analyze k,d and rsi values
	private static void runAnalysis(String myStock, List<RSIPrice> rsiPriceList) {
		...
		System.out.println("Run analysis is called for " + myStock + " with size of list"+rsiPriceList.size());		
...
//			        )
	...
	return spxInfo
	.stream()
	.filter( x -> x.getDateTime().toString().substring(0,10).equals(data.getPriceDateTime().toString().substring(0, 10)))
	.count() > 0;
}
	/**
	 * @param timeSeries
	...
	private static List<RSIData> rsiResult(
	    ...
	    List<RSIData> rsiData = new ArrayList<RSIData>();
		try {
	...
	private static List<StockData> dailyStockData(TimeSeries timeSeries,
		...
		Thread.sleep(SLEEP_INTERVAL/ DENOMINATOR);
		DailyAdjusted dailyAdjusted = timeSeries.dailyAdjusted(myStock,OutputSize.FULL);
	...
	private static List<STOCHDataSlow> stochResult(
			...
			throws InterruptedException {
		Thread.sleep(SLEEP_INTERVAL/ DENOMINATOR);
		List<STOCHDataSlow> stochData = new ArrayList<>();
		try {
			STOCH stockDataSlowResponse = technicalIndicators.stoch(myStock.trim(),Interval.DAILY , FastKPeriod.of(14), SlowKPeriod.of(3), SlowDPeriod.of(3), SlowKMaType.SMA, SlowDMaType.SMA);
			stochData  = stockDataSlowResponse
					.getData()
					.stream()
					.filter(x -> x.getDateTime().isAfter(
							LocalDateTime.parse(MyRSI.START_DATE))
	...
	private static List<RSIData> collectRsiData(String myStock) throws Exception { 
	return rsiResult(technicalIndicators,
			myStock).stream().collect(Collectors.toList());
	}
	
	private static List<StockData> collectPriceData(String myStock) throws Exception {
	return dailyStockData(timeSeries,
			myStock).stream().collect(Collectors.toList());
	}
	
	private static List<STOCHDataSlow> collectSTOCHData(String myStock) throws Exception {
	return stochResult(technicalIndicators,
			myStock).stream().collect(Collectors.toList());
	}

	private static BiFunction<RSIPrice, RSIPrice, Double> gain = (current,oldest) -> 
	{
	double roi = ((current.getPrice() - oldest.getPrice()) / oldest
			.getPrice()) * 100;
	System.out.println(current.getStock() + ":" + roi
			+ "%. Current RSI:" + current.getRsi()
            + ":date:"+ oldest.getPriceDateTime().toString().substring(0,10)				
			+ ":Older rsi:" + oldest.getRsi()
			+ ":Older k:"+ oldest.getSlowK()
			+ ":Older d:"+ oldest.getSlowD()
			);
	return roi;
	};

	private static List<RSIData> getSPXInfo()throws Exception{
		return collectRsiData("SPX")
		.stream()
